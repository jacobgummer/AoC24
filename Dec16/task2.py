...
def dijkstra(maze: np.ndarray, start_pos: tuple[int, int], end_pos: tuple[int, int]):
    ...
    heapq.heappush(pq, (0, start_pos, '>', [start_pos]))

    while pq:
        cost, p, direction, path = heapq.heappop(pq)
        
        if cost > dist.get((p, direction), inf):
            continue
        dist[p, direction] = cost
        
        if p == end_pos and cost <= best:
            seen += path
            best = cost
        ...
        for new_dir, next_p in neighbours:
            new_cost = 1001 if new_dir != direction else 1
            heapq.heappush(pq, (new_cost + cost, next_p, new_dir, [next_p] + path))
    
    return set(seen)
        
maze, start, end = parse_lines(lines)
seen = dijkstra(maze, start, end)
print(len(seen))